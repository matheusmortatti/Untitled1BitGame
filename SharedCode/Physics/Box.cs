...
namespace SharedCode.Physics {
	public class Box {
		...
		public List<Box> CheckCollision(out Vector2 separationVector) {
			...
			Map mapInstance = GameObjectManager.FindObjectOfType<Map>() as Map;
			if (mapInstance == null)
				goto cleanup;
			...
			foreach (Vector2 p in possibleTileCollisions) {
				...
				if (mapInstance.IsSolid(new Vector2((int)Math.Floor(tileBox.position.X / 8), (int)Math.Floor(tileBox.position.Y / 8)))) {
